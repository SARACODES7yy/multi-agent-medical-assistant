...
class AgentConfig:
    ...
    # Decision model
    DECISION_MODEL = "gemini-3.6-flash"  # or whichever model you prefer
    ...
    4. MEDICAL_IMAGE_AGENT - For analysis of uploaded medical images, lab reports, prescriptions, or other health documents (extracts structured information, never diagnoses).

    Make your decision based on these guidelines:
    - If the user has not uploaded any image, always route to the conversation agent.
    - If the user uploads a medical image or document, route to the MEDICAL_IMAGE_AGENT to extract structured information. If the image is uploaded without a query, always route to the MEDICAL_IMAGE_AGENT.
    ...
    - For general conversation, greetings, or non-medical questions, use the conversation agent. But if image is uploaded, always go to the medical image agent first.
...
def create_agent_graph():
    ...
    def analyze_input(state: AgentState) -> AgentState:
        ...
        if isinstance(current_input, dict) and "image" in current_input:
            ...
            # Skip classification to reduce latency when an image is explicitly uploaded
            image_type = "MEDICAL_REPORT"
            print("ANALYZED IMAGE TYPE: ", image_type, "(classification skipped)")
    ...
    def route_to_agent(state: AgentState) -> Dict:
        ...
        # When an image is attached, route directly to the medical image agent
        if has_image:
            print(f"Decision (image bypass): MEDICAL_IMAGE_AGENT")
            updated_state = {**state, "agent_name": "MEDICAL_IMAGE_AGENT"}
            return {"agent_state": updated_state, "next": "MEDICAL_IMAGE_AGENT"}
    ...
    def run_rag_agent(state: AgentState) -> AgentState:
        ...
        try:
            ...
            print(f"RAG_AGENT init failed ({e}); falling back to LLM")
            rag_agent = None
        ...
        if rag_agent is None:
            response_text = str(config.rag.response_generator_model.invoke(query).content)
            retrieval_confidence = 0.0
            response = {"confidence": retrieval_confidence, "sources": [], "response": response_text}
        else:
    ...
    def run_medical_image_agent(state: AgentState) -> AgentState:
        """Handle uploaded medical images / reports using Gemini vision."""
        print(f"Selected agent: MEDICAL_IMAGE_AGENT")

        current_input = state["current_input"]
        image_path = current_input.get("image", None)
        text = current_input.get("text", "")

        # Use Gemini vision to extract structured medical information
        response = AgentConfig.image_analyzer.analyze_medical_image(image_path, extra_context=text)
        response_message = AIMessage(content=response)

        return {
            **state,
            "output": response_message,
            "needs_human_validation": True,  # Medical information always needs review
    ...
    workflow.add_node("MEDICAL_IMAGE_AGENT", run_medical_image_agent)
    ...
    workflow.add_conditional_edges(
        ...
        lambda x: x["next"],
        {
            ...
            "MEDICAL_IMAGE_AGENT": "MEDICAL_IMAGE_AGENT",
    ...
    workflow.add_edge("MEDICAL_IMAGE_AGENT", "check_validation")
    ...
    # Compile the graph
    return workflow.compile(checkpointer=memory, interrupt_before=["apply_guardrails"])
...
def process_query(query: Union[str, Dict], conversation_history: List = None, user_id: str = None):
    """
    Process a user query through the agent decision system.

    Args:
        query: User input (text string or dict with text and image)
        conversation_history: Optional list of dicts {role, content} from persisted chat history
        user_id: Optional logged-in user id (used as the LangGraph thread id)

    Returns:
        dict with either status=success (messages, agent_name) or status=validation_required
    """
    graph = get_graph()
    # Use a fresh thread per invocation so a stale interrupted checkpoint is never resumed.
    # Conversation memory comes from the DB-seeded history, not the LangGraph checkpointer.
    thread_id = f"{user_id or 'anon'}-{uuid.uuid4().hex[:8]}"
    thread_cfg = {"configurable": {"thread_id": thread_id}}

    # Initialize state
    state = init_agent_state()

    # Seed persisted history into the state (conversation memory)
    messages = []
    ...
    result = graph.invoke(state, thread_cfg)
    state_snapshot = graph.get_state(thread_cfg)
    if state_snapshot.next:
        needs_val = state_snapshot.values.get("needs_human_validation", False)
        output_msg = state_snapshot.values.get("output")
        thread_id = thread_cfg["configurable"]["thread_id"]
        if needs_val:
            validation_text = output_msg.content if isinstance(output_msg, BaseMessage) else (str(output_msg) if output_msg else "Please validate this output.")
            return {"status": "validation_required", "message": validation_text, "thread_id": thread_id}
        # No human validation needed (e.g. text chat): complete normally with the produced output
        response_text = output_msg.content if isinstance(output_msg, BaseMessage) else (str(output_msg) if output_msg else "I couldn't find enough information to answer that.")
        return {"status": "success", "messages": [AIMessage(content=response_text)], "agent_name": state_snapshot.values.get("agent_name", "") or "SYSTEM"}

    # Keep history to reasonable size (ANOTHER OPTION: summarize and store before truncating history)
    messages = result.get("messages", state_snapshot.values.get("messages", []))
    ...
    return {"status": "success", "messages": messages, "agent_name": result.get("agent_name", "")}