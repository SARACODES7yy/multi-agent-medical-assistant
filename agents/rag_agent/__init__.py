...
class MedicalRAG:
    ...
    def __init__(self, config):
        ...
        try:
            ...
            # Disable if reranking explicitly off or dependencies (torch) unavailable
            if not getattr(config.rag, "enable_reranker", False) or not self.reranker.available:
                self.reranker = None
        except Exception as e:
            self.logger.warning(f"Reranker unavailable, continuing without it: {e}")
            self.reranker = None
